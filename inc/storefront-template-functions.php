...
if ( ! function_exists( 'storefront_comment' ) ) {
	...
	function storefront_comment( $comment, $args, $depth ) {
		...
		<div class="comment-meta commentmetadata">
			...
			<?php printf( wp_kses_post( '<cite class="fn">%s</cite>', 'noakirel' ), get_comment_author_link() ); ?>
			</div>
			<?php if ( '0' === $comment->comment_approved ) : ?>
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'noakirel' ); ?></em>
				<br />
			<?php endif; ?>
		...
		?>
		<?php edit_comment_link( __( 'Edit', 'noakirel' ), '  ', '' ); ?>
		</div>
		</div>
		<?php if ( 'div' !== $args['style'] ) : ?>
		</div>
	...
	function storefront_credit() {
		...
		if ( apply_filters( 'storefront_credit_link', true ) ) {
			if ( storefront_is_woocommerce_activated() ) {
				$links_output .= '<a href="https://woocommerce.com" target="_blank" title="' . esc_attr__( 'WooCommerce - The Best eCommerce Platform for WordPress', 'noakirel' ) . '" rel="noreferrer nofollow">' . esc_html__( 'Built with WooCommerce', 'noakirel' ) . '</a>.';
			} else {
				$links_output .= '<a href="https://woocommerce.com/products/storefront/" target="_blank" title="' . esc_attr__( 'Storefront -  The perfect platform for your next WooCommerce project.', 'noakirel' ) . '" rel="noreferrer nofollow">' . esc_html__( 'Built with Storefront', 'noakirel' ) . '</a>.';
			}
		}
...
if ( ! function_exists( 'storefront_primary_navigation' ) ) {
	...
	function storefront_primary_navigation() {
		?>
		<nav id="site-navigation" class="main-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Primary Navigation', 'noakirel' ); ?>">
		<button id="site-navigation-menu-toggle" class="menu-toggle" aria-controls="site-navigation" aria-expanded="false"><span><?php echo esc_html( apply_filters( 'storefront_menu_toggle_text', __( 'Menu', 'noakirel' ) ) ); ?></span></button>
			<?php
			wp_nav_menu(
				array(
...
if ( ! function_exists( 'storefront_secondary_navigation' ) ) {
	...
	function storefront_secondary_navigation() {
		if ( has_nav_menu( 'secondary' ) ) {
			?>
			<nav class="secondary-navigation" role="navigation" aria-label="<?php esc_attr_e( 'Secondary Navigation', 'noakirel' ); ?>">
				<?php
					wp_nav_menu(
						array(
...
if ( ! function_exists( 'storefront_skip_links' ) ) {
	...
	function storefront_skip_links() {
		?>
		<a class="skip-link screen-reader-text" href="#site-navigation"><?php esc_html_e( 'Skip to navigation', 'noakirel' ); ?></a>
		<a class="skip-link screen-reader-text" href="#content"><?php esc_html_e( 'Skip to content', 'noakirel' ); ?></a>
		<?php
	}
}
...
if ( ! function_exists( 'storefront_homepage_header' ) ) {
	...
	function storefront_homepage_header() {
		edit_post_link( __( 'Edit this section', 'noakirel' ), '', '', '', 'button storefront-hero__button-edit' );
		?>
	...
	function storefront_page_content() {
		...
		<div class="entry-content">
			...
			<?php
				wp_link_pages(
					array(
						'before' => '<div class="page-links">' . __( 'Pages:', 'noakirel' ),
						'after'  => '</div>',
					)
				);
			?>
	...
	function storefront_post_content() {
		...
		the_content(
			sprintf(
				...
				__( 'Continue reading %s', 'noakirel' ),
...
if ( ! function_exists( 'storefront_post_meta' ) ) {
	...
	function storefront_post_meta() {
		...
		$posted_on = '
			...
			sprintf( __( 'Posted on %s', 'noakirel' ), $output_time_string ) .
			'</span>';
		...
		if ( ! post_password_required() && ( comments_open() || 0 !== intval( get_comments_number() ) ) ) {
			$comments_number = get_comments_number_text( __( 'Leave a comment', 'noakirel' ), __( '1 Comment', 'noakirel' ), __( '% Comments', 'noakirel' ) );
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'noakirel' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'storefront_post_taxonomy' ) ) {
	...
	function storefront_post_taxonomy() {
		...
		$categories_list = get_the_category_list( __( ', ', 'noakirel' ) );

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'noakirel' ) );
		?>

		<aside class="entry-taxonomy">
			<?php if ( $categories_list ) : ?>
			<div class="cat-links">
				<?php echo esc_html( _n( 'Category:', 'Categories:', count( get_the_category() ), 'noakirel' ) ); ?> <?php echo wp_kses_post( $categories_list ); ?>
			</div>
			<?php endif; ?>

			<?php if ( $tags_list ) : ?>
			<div class="tags-links">
				<?php echo esc_html( _n( 'Tag:', 'Tags:', count( get_the_tags() ), 'noakirel' ) ); ?> <?php echo wp_kses_post( $tags_list ); ?>
			</div>
			<?php endif; ?>
		</aside>
...
if ( ! function_exists( 'storefront_paging_nav' ) ) {
	...
	function storefront_paging_nav() {
		...
		$args = array(
			'type'      => 'list',
			'next_text' => _x( 'Next', 'Next post', 'noakirel' ),
			'prev_text' => _x( 'Previous', 'Previous post', 'noakirel' ),
		);
...
if ( ! function_exists( 'storefront_post_nav' ) ) {
	...
	function storefront_post_nav() {
		$args = array(
			'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next post:', 'noakirel' ) . ' </span>%title',
			'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous post:', 'noakirel' ) . ' </span>%title',
		);
