...
if ( ! class_exists( 'noakirel' ) ) :
	...
	class Storefront {
		...
		public function setup() {
			...
			load_theme_textdomain( 'noakirel', trailingslashit( WP_LANG_DIR ) . 'themes' );

			// Loads wp-content/themes/child-theme-name/languages/it_IT.mo.
			load_theme_textdomain( 'noakirel', get_stylesheet_directory() . '/languages' );

			// Loads wp-content/themes/storefront/languages/it_IT.mo.
			load_theme_textdomain( 'noakirel', get_template_directory() . '/languages' );
			...
			register_nav_menus(
				apply_filters(
					...
					array(
						'primary'   => __( 'Primary Menu', 'noakirel' ),
						'secondary' => __( 'Secondary Menu', 'noakirel' ),
						'handheld'  => __( 'Handheld Menu', 'noakirel' ),
					)
				)
			);
			...
			add_theme_support(
				...
				array(
					array(
						'name' => __( 'Small', 'noakirel' ),
						'size' => 14,
						'slug' => 'small',
					),
					array(
						'name' => __( 'Normal', 'noakirel' ),
						'size' => 16,
						'slug' => 'normal',
					),
					array(
						'name' => __( 'Medium', 'noakirel' ),
						'size' => 23,
						'slug' => 'medium',
					),
					array(
						'name' => __( 'Large', 'noakirel' ),
						'size' => 26,
						'slug' => 'large',
					),
					array(
						'name' => __( 'Huge', 'noakirel' ),
						'size' => 37,
						'slug' => 'huge',
					),
				)
		...
		public function widgets_init() {
			$sidebar_args['sidebar'] = array(
				'name'        => __( 'Sidebar', 'noakirel' ),
				'id'          => 'sidebar-1',
				'description' => '',
			);

			$sidebar_args['header'] = array(
				'name'        => __( 'Below Header', 'noakirel' ),
				'id'          => 'header-1',
				'description' => __( 'Widgets added to this region will appear beneath the header and above the main content.', 'noakirel' ),
			);
			...
			for ( $row = 1; $row <= $rows; $row++ ) {
				for ( $region = 1; $region <= $regions; $region++ ) {
					...
					if ( 1 === $rows ) {
						...
						$footer_region_name = sprintf( __( 'Footer Column %1$d', 'noakirel' ), $region );

						/* translators: 1: column number */
						$footer_region_description = sprintf( __( 'Widgets added here will appear in column %1$d of the footer.', 'noakirel' ), $region );
					} else {
						/* translators: 1: row number, 2: column number */
						$footer_region_name = sprintf( __( 'Footer Row %1$d - Column %2$d', 'noakirel' ), $row, $region );

						/* translators: 1: column number, 2: row number */
						$footer_region_description = sprintf( __( 'Widgets added here will appear in column %1$d of footer row %2$d.', 'noakirel' ), $region, $row );
					}
		...
		public function scripts() {
			...
			if ( has_nav_menu( 'handheld' ) ) {
				$storefront_l10n = array(
					'expand'   => __( 'Expand child menu', 'noakirel' ),
					'collapse' => __( 'Collapse child menu', 'noakirel' ),
				);
		...
		public function navigation_markup_template() {
			$template  = '<nav id="post-navigation" class="navigation %1$s" role="navigation" aria-label="' . esc_html__( 'Post Navigation', 'noakirel' ) . '">';
